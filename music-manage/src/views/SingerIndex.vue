<template>
  <div>
    <div class="top">
      <div class="search">
        ...
        <el-button
          ...
          >新增歌手</el-button
        >
        <el-button type="primary" round onclick="window.print()"
          >打印预览</el-button
        >
    ...
    <el-table :data="tableData" style="width: 100%">
      ...
      <el-table-column prop="location" label="故乡" width="180">
      </el-table-column>
      <el-table-column label="查看歌曲">
        <template slot-scope="scope">
          <el-button size="mini" @click="searchsong(scope.row.id)"
            >查看歌曲</el-button
          >
        </template>
      </el-table-column>
  ...
  methods: {
    ...
    search() {
      axios
        ...
        .then((res) => {
          if (res.data.code === 0) {
            this.tableData = res.data.data.singerList;
          }
        });
    },
    ...
    async handleDelete(id) {
      this.$confirm("你确定要删除吗?", "提示", {
        ...
        await axios.delete(`/admin/deleteSinger/${id}`).then((res) => {
          if (res.data.code === 0) {
